<template>
  <!-- <el-card> -->
    <el-dialog iconClass="el-icon-info" title="编辑分类" width="500px" :visible.sync="isDialog" 
      ...
      <el-form :model="innerItem" :rules="rules" ref="innerItem" label-position="right" label-width="100px">
        ...
        <el-form-item label="头像：" prop="file">
          <el-upload action="https://jsonplaceholder.typicode.com/posts/" :show-file-list="false" :http-request="uploadSectionFile">
            <div class="src" v-if="innerItem.file">
              <img :src="innerItem.file" class="avatar">
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        file: [{ required: true, message: '请上传缩略图' }],
  ...
  created() {
    this.innerItem.file=this.item.file;
  ...
  methods: {
    ...
    uploadSectionFile: function(param) {
      ...
      this.$http
        ...
        .then(res => {
          this.innerItem.file = res.imgurl
        }).catch(()=>{})
    },
    ...
    submit() {
      this.$refs.innerItem.validate(valid => {
        if (!valid) return
          ...
          this.$http.post('/manage/updata', {file:this.innerItem.file,name:this.innerItem.name,age:this.innerItem.age,sex:this.innerItem.sex,id:this.innerItem.id}).then(res => {
              this.$emit('close', '1')
          }).catch(()=>{})
